...
module.exports = function (server, app, session) {
  ...
  server.get('/hadiah', (req, res) => {
    ...
    if (req.session.accessToken != undefined && req.session.accessToken != null && req.session.accessToken != '') {
      if (isDesktop) return app.render(req, res, '/hadiah');
      else return app.render(req, res, '/mobiles/hadiah');
    } else {
      res.redirect('/auth');
    }
  });

  server.get('/hadiah/:id', (req, res) => {
    const isDesktop = req.useragent.isDesktop;
    const apiParams = req.params;

    if (req.session.accessToken != undefined && req.session.accessToken != null && req.session.accessToken != '') {
      if (isDesktop) return app.render(req, res, '/hadiah/' + apiParams.id);
      else return app.render(req, res, '/mobiles/hadiah/' + apiParams.id);
    } else {
      res.redirect('/auth');
    }
  });

  server.get('/hadiah/form-redeem/:id', (req, res) => {
    const isDesktop = req.useragent.isDesktop;
    const apiParams = req.params;

    if (req.session.accessToken != undefined && req.session.accessToken != null && req.session.accessToken != '') {
      if (isDesktop) return app.render(req, res, '/hadiah/form-redeem/' + apiParams.id);
      else return app.render(req, res, '/mobiles/hadiah/form-redeem/' + apiParams.id);
    } else {
      res.redirect('/auth');
    }
  });
  ...
  //==================================================================//
  server.post('/redeem', reqTime, (req, res) => {
    ...
    if (sessionAccessToken !== null) {
      ...
      axios.get(config.apiHost + '/v1/redeem',
        {
          responseType: 'json',
          crossdomain: true,
          headers: {
  ...
  server.post('/redeem/:id', reqTime, (req, res) => {
    const apiParams = req.params;
    ...
    if (sessionAccessToken !== null) {
      axios.get(config.apiHost + `/v1/redeem/${apiParams.id}`,
        {
          responseType: 'json',
          crossdomain: true,
          headers: {
  ...
  server.post('/redeem/:id/upload-ktp', upload.single('file'), (req, res) => {
    ...
    if (sessionAccessToken !== null) {
      ...
      axios.post(`${config.apiHost}/v1/redeem/${apiParams.id}/upload-ktp`,
        bodyFormData,
        {
          responseType: 'json',
          crossdomain: true,
  ...
  server.post('/redeem/:id/redeem', reqTime, (req, res) => {
    const apiParams = req.params;
    ...
    if (sessionAccessToken !== null) {
      ...
      axios.post(`${config.apiHost}/v1/redeem/${apiParams.id}/redeem`,
        req.body,
        {
          responseType: 'json',
          crossdomain: true,
