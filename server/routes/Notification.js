...
module.exports = function (server, app, session) {
    ...
    server.get('/notification', (req, res) => {
        ...
        if (req.session.accessToken != undefined && req.session.accessToken != null && req.session.accessToken != '') {
            if (isDesktop) return app.render(req, res, '/notification');
            else return app.render(req, res, '/mobiles/notification');
        } else {
            res.redirect('/auth');
        }
    });
    ...
    //==================================================================//
    server.post('/notification', reqTime, (req, res) => {
        ...
        if (sessionAccessToken !== null) {
            ...
            axios.get(config.apiHost + '/v1/notification',
                {
                    responseType: 'json',
                    crossdomain: true,
                    headers: {
                ...
                .then((response) => {
                    // console.log('notification.response: ' + JSON.stringify(response.data.data));
    ...
    server.post('/notification/:id/read', reqTime, (req, res) => {
        ...
        if (sessionAccessToken !== null) {
            ...
            axios.get(config.apiHost + '/v1/notification/' + apiParams.id + '/read',
                {
                    responseType: 'json',
                    crossdomain: true,
                    headers: {
                ...
                .then((response) => {
                    // console.log('notificationRead.response: ' + JSON.stringify(response.data.data));
                    if (response.status === 200) {
                        const resData = response.data.data;
                ...
                .catch(error => {
                    // console.log('notificationRead.error: ' + JSON.stringify(error));
                    if (error.response !== undefined && error.response.data !== null) {
                        res.status(error.response.status).json(retCode.ret422(req, res, { _error: 1, message: error.message }));
                    } else {
                        res.json(retCode.ret500(req, res, { error }));
                    }
                });
        }
    });
