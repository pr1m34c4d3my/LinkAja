...
export default function CardGroupPost({ data }) {

  return (
    <Link href={`/group/post/[id]?id=${data.id}`} as={`/group/post/${data.id}`}>
      <a className="no-underline hover:no-underline p-3 flex rounded-lg border border-gray-200 mb-3">
        <div className="mr-4">
          <div className="rounded-lg" style={{ width: '64px', height: '64px' }}>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center">
            ...
            <div className="text-gray-400">{data.comment_count} Balasan</div>
          </div>
        </div>
      </a>
    </Link>
