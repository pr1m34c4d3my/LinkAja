...
export default function GroupPost({
  ...
  const __apiCommentPost = async (comment) => {
    if (post != null) {
      return axios.get(`/group/post/${post.id}/reply`,
        {
          responseType: 'json',
          crossdomain: true,
          headers: {
        ...
        .then((response) => {
          // console.log('dataGroupPost.response: ' + JSON.stringify(response.data.data));
          if (response.status === 200) {
            const resData = response.data.data;
        ...
        .catch(error => {
          // console.log('dataGroupPost.error: ' + JSON.stringify(error));
          if (error.response !== undefined && error.response.data !== null) {
            return { _error: 1, message: error.message };
          } else {
            return { _error: 1, message: error.message };
      ...
      <div id="post_share">
        <ul className="flex flex-row items-center">
          <li className="w-8 h-8">
            <FacebookShareButton url={contentURL} quote={groupTitle}>
              <FacebookIcon size={28} round={true} />
            </FacebookShareButton>
          </li>
          <li className="w-8 h-8">
            <TwitterShareButton url={contentURL} title={groupTitle}>
              <TwitterIcon size={28} round={true} />
            </TwitterShareButton>
          </li>
          <li className="w-8 h-8">
            <WhatsappShareButton url={contentURL} title={groupTitle}>
              <WhatsappIcon size={28} round={true} />
            </WhatsappShareButton>
          </li>
          <li className="w-8 h-8">
            <LinkedinShareButton url={contentURL} title={groupTitle}>
  ...
  const _renderPost = () => {
    return (
      <>
        <div className="w-full h-30 mb-5 overflow-hidden">
    ...
    return (
      <div className="mb-3">
        <Breadcrumb separator=">">
          ...
          <Breadcrumb.Item>
            <Link href={`/group/category/[id]?id=${post.Group.id}`} as={`/group/${post.Group.id}`}>
              <a>{post.Group.title}</a>
            </Link>
          </Breadcrumb.Item>
