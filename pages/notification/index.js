...
export default function Notification() {
          ...
          <div className="flex items-center">
            ...
            <div className='ml-1'>
              <span className='text-xs text-gray-300 font-normal'>
                <div className='block md:hidden lg:hidden xl:hidden'>{parseTimeAgoMini(new Date(item.created_at))}</div>
                <div className='hidden md:block lg:block xl:block'>{parseTimeAgo(new Date(item.created_at))}</div>
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <Layout title='Notifikasi'>
      <div className='m-4'>
        <h3 className='font-black text-gray-600 text-xl'>Notifikasi</h3>
      </div>
      <div className='bg-white p-4 rounded-xl mb-3'>
        {notifications ? _renderNotificationList() : <Skeleton count={10} />}
      </div>
    </Layout >
  )
}
